...
import json, signal, time, threading, asyncio
...
def consumeResponse(message, exchange):
    with message.process():
        ...
        __ssNumbers[ssn].append({'ssn':ssn, 'interestRate':interestRate})
        log(dict)
...
async def toBrokerOut(outdict, exchange):
    await exchange.publish(aio_pika.Message(body = json.dumps(outdict).encode()), routing_key='bestquote')
    return
...
def bestLoanDecision(ssn, timeout):
        ...
        bestRate = None
        for d in __ssNumbers[ssn]:
            if bestRate == None or bestRate['interestRate'] < d['interestRate']:
                bestRate = d
...
async def main(loop):
    ...
    tempinex = channel.declare_exchange(__inputexchangename, ExchangeType.DIRECT, passive=True, durable=True)
    tempoutex = channel.declare_exchange(__outputexchangename, ExchangeType.FANOUT, passive=True, durable=True)
    tempin_request = channel.declare_queue(auto_delete=True, exclusive=True)
    tempin_response = channel.declare_queue(auto_delete=True, exclusive=True)
    ...
    try:
        ...
        await asyncio.wait([requestL, responseL])
